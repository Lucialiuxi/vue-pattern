<template>
    <div>
        ...
        <Page :total="goodData.length" @on-change="changePage" :page-size="10"></Page>
    </div>
</template>
<script>
...
export default {
  ...
  beforeRouteEnter(to,from,next){
      ...
      next(function(vm){
        vm.$store.dispatch('getGoodDataAction')
      })
  }
  ,
  methods:{
  ...
  computed:{
      ...
      filterGoodData(){
          let arr = this.goodData.filter((item,index)=> index>=this.beginPage && index<=this.endPage)
        //   console.log(arr)
          return arr
