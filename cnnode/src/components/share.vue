<template>
    <div>
        ...
        <Page :total="shareData.length" @on-change="changePage" :page-size="10"></Page>
    </div>
</template>
<script>
...
export default {
  ...
  beforeRouteEnter(to,from,next){
      ...
      next(function(vm){
        vm.$store.dispatch('getShareDataAction')
      })
  }
  ,
  methods:{
  ...
  computed:{
      ...
      filterShareData(){
          let arr = this.shareData.filter((item,index)=> index>=this.beginPage && index<=this.endPage)
        //   console.log(arr)
          return arr
