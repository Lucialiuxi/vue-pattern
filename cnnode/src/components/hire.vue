<template>
    <div>
        ...
        <Page :total="jobData.length" @on-change="changePage" :page-size="10"></Page>
    </div>
</template>
<script>
...
export default {
  ...
  beforeRouteEnter(to,from,next){
      ...
      next(function(vm){
        vm.$store.dispatch('getJobDataAction')
      })
  }
  ,
  methods:{
  ...
  computed:{
      ...
      filterJobData(){
          let arr = this.jobData.filter((item,index)=> index>=this.beginPage && index<=this.endPage)
        //   console.log(arr)
          return arr
